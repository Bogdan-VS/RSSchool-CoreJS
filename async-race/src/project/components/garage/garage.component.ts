    ...
    for (let i = 0; i < data.length; i++) {
      ...
      carConteiner.innerHTML = `
        <div class="car-container-title">
          <button class="title-car-btn" data-select="${i}">Select</button>
          <button class="title-car-btn" data-remove="${i}">Select</button>
          <h3 class="title-car-name">${data[i].name}</h3>
        </div>
        <div class="car-container-subtitle">
          <button class="subtitle-car-btn" data-start="${i}">A</button>
          <button class="subtitle-car-btn" data-stop="${i}">B</button>
          <div class="track">
            <svg display="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">
              <symbol id="car${i}" viewBox="0 0 100 42.591">
      ...
      garageCarsConteiner.append(carConteiner);
    }
  }
}